...
class SolarSystemBodies:
    ...
    def draw_body(self,WINDOW):
        ...
        y = self.x*SolarSystemBodies.SCALE + HEIGHT//2
        ...
        if not self.sun:
            ...
            dist_text = DIST_TEXT.render(f"{round(self.distance_to_sun/(3e8*60), 3 )}, 1t-min", True, NAME_TEXT_COLOR)
            WINDOW.blit(dist_text, (x-40, y-35))
        else:
            name_text = NAME_TEXT.render(self.name, True, SUN_NAME_COLOR)
            WINDOW.blit(name_text, (x-40, y-78))
            dist_text = DIST_TEXT.render(f"{round(self.x/3e8, 3), round(self.y/3e8, 3)} 1t-sec",True, DIST_TEXT_COLOR)
    ...
    # Method 2 - Calculate the Gravatational Force
...
while run:
        ...
        if not paused:            
            ...
            for body in ss_bodies:
                ...
                body.draw(WINDOW, track=False)   # change --> track=True  .. . if you want to track the orbit
            pg.display.update()
